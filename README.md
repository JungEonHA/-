...
- **iframe 저장소.** 브라우저는 iframe 안의 localStorage 를 상위 사이트별로 쪼개 두거나
  (Chrome·Firefox) 아예 막는다(Safari 기본값). 쪼개는 것은 문제가 없다 — 사람마다
  자기 브라우저에 자기 기록이 남으므로 오히려 의도한 동작이다. 막히는 경우는 앱이
  감지해 위젯 상단에 붉은 경고를 띄우고 **전체 화면 ↗** 로 새 탭에서 쓰도록 안내한다.
- **같은 이유로 임베드와 새 탭은 서로 다른 저장소를 쓴다.** 설정은 각각 한 번씩 해야
  한다. 근무 기록 자체는 Notion 을 통해 합쳐지므로(§6-2) 문제가 되지 않는다.
