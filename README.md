...
기록은 브라우저 로컬에 쌓이므로 **한 기기(브라우저) = 한 사람**이 전제다.
설정 → 직원에서 이 기기를 쓰는 사람을 고르면, 그 값이 Notion `직원` Property 로 기록되고
행을 가르는 두 번째 기준이 된다. 직원 필드를 매핑해 놓고 사람을 고르지 않으면
동기화를 **멈춘다** — 누구 것인지 모르는 행을 만들어 서로의 기록을 섞는 것보다 낫다.

선택지는 Notion `직원` Property 의 실제 옵션에서 읽어 온다. 사람이 늘면 Notion 에서
옵션을 추가하고 "스키마 다시 읽기"만 누르면 된다 (앱 배포 불필요).

### 6-2. 퇴근 취소 — `resume` 이벤트
